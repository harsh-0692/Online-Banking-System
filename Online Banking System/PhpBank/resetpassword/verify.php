...
if (isset($_POST['send'])) {
    ...
    if ($find_employee) {
        ...
        if ($result['email_address'] == $user_mail) {
            ...
            $find_already = "DELETE `otp` FROM user_otp WHERE email_address='$user_mail'";
            ...
            $current_time = time();
            #echo $current_time;
            $otp_expire_time = $current_time + 600; #expire after 2min
            
           
            //add this hash to the database
            $sql = "INSERT INTO `user_otp` (`email_address`, `otp`,`otp_expire_time`) VALUES ('$user_mail', '$OTP','$otp_expire_time')";
            ...
            //now send a mail
            $send_mail_check = mail($to_email,$subject,$message,$header);
